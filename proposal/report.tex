...
\todo[color=mygreen]{\large Latest progress can be found in Section
...
\end{abstract}
	...
	\begin{multicols}{2}
		...
		\includegraphics[width=0.9\linewidth]{cookie-theft-picture.png}
...
\todo{write summary (see notebook)}
\begin{figure*}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{dataset.png}
	\caption{\todo{write caption.}} \label{fig:dataset}
\end{figure*}


\section{Methodology}\label{sec:methodology}
\todo{what has happened so far}
\subsection{Baseline Experiments}
\todo{todo}
\subsection{Implementation Characteristics}
\todo{Include both semantic and paralinguistic modalities, focus on conversational modeling etc}
\subsection{Conversational Modeling}
\todo{mention options}

\section{Future Steps}
\begin{itemize}
	\item Finish MultiConAD baseline.
	\item Replicate existing paper on MultiConAD.
	\item List available conversational modeling options.
\end{itemize}
