...
\new{\section{Methodology}}\label{sec:methodology}
...
\note{Maybe I could try fusing with a Cross-Attention Transformer after the graph modeling?}
...
\end{enumerate} 
\note{Maybe also add the original question as a node in the context, like
\cite{liGLGRQuestionawareGlobaltoLocal2023}? How could this be achieved?}
...
\note{attention+RGCN vs RGAT}

\tpar{Pooling \& Classification} For the classification step, we will perform
attention pooling over all patient utternaces to extract a patient-wide
representation, which will then be imposed to the final classification. This
selective process prevents the model from directly classifying interviewer
speech and instead focusing on the participant's speech and its interactions in
the dialogue.
...
\begin{enumerate}
	...
	\item 
		...
		\emph{4,000 hours} of dialogues between \emph{4000+ participants}. 
\end{enumerate}
\note{What pretraing algorithm could we use?}
